...
const ABC_CONFIG: Record<string, { label: string; color: string; bg: string; border: string; desc: string }> = {
  A: { label: 'Clase A', color: 'hsl(var(--primary))', bg: 'bg-primary/10', border: 'border-primary/20', desc: 'Alto valor — Control estricto y revisión semanal' },
  B: { label: 'Clase B', color: 'hsl(var(--warning))', bg: 'bg-warning/10', border: 'border-warning/20', desc: 'Valor medio — Control periódico quincenal' },
  C: { label: 'Clase C', color: 'hsl(var(--muted-foreground))', bg: 'bg-muted/50', border: 'border-border', desc: 'Bajo valor — Control básico mensual' },
}
...
export default function AbcPage() {
  ...
  if (loading || !data) return <SkeletonKPIsAndTable cols={8} rows={6} />
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        subtitle={`Clasificación Pareto · ${items.length} items · Valor total: ${fmtEur(totalValue)}`}
        lastUpdated={lastUpdated}
        onRefresh={handleRefresh}
        actions={
          <Button variant="outline" size="sm" onClick={() => exportCSV('inventario_abc',
            ['SKU', 'Descripción', 'Clase', 'Stock', 'Coste Unit.', 'Valor Total', '% Valor', '% Acumulado', 'Rotación', 'Stock Seguridad', 'Punto Reorden', 'Recomendación'],
        ...
        <div className="flex items-start gap-3 p-4 rounded-lg border border-warning/30 bg-warning/10 text-warning">
          ...
          <div>
            {reorderUrgent.length > 0 && (
              <div className="font-semibold text-sm">{reorderUrgent.length} item{reorderUrgent.length > 1 ? 's' : ''} bajo stock de seguridad — Reposición urgente</div>
            )}
            {reorderNeeded.length > 0 && (
              <div className="text-xs opacity-80 mt-0.5">{reorderNeeded.length} item{reorderNeeded.length > 1 ? 's' : ''} por debajo del punto de pedido</div>
            )}
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Pareto chart */}
        <Card>
          <CardHeader><CardTitle>Curva de Pareto</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={paretoData} margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v: number, name: string) => [name === 'cumPct' ? `${v}%` : fmtEur(v), name === 'cumPct' ? '% Acumulado' : 'Valor']}
                />
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={summaryBars} layout="vertical" margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                ...
                <Tooltip
                  contentStyle={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: 8, fontSize: 11 }}
                  formatter={(v: number) => [fmtEur(v), 'Valor']}
                />
            ...
            <div className="mt-3 space-y-2">
              {summaryBars.map(b => (
                <div key={b.name} className="flex items-center gap-3 text-xs">
                  ...
                  <span className="flex-1 text-muted-foreground">{b.items} items · {fmtPct(b.pct)} del valor</span>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                {['Clase', 'SKU', 'Descripción', 'Stock', 'Valor Total', '% Valor', '% Acum.', 'Rotación', 'Recomendación'].map(h => (
                        ...
                        <td colSpan={9} className="p-4 bg-muted/30">
                          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
                            <div className="bg-card rounded-lg p-3 border border-border">
                              <div className="text-[10px] text-muted-foreground uppercase">Coste Unitario</div>
                              <div className="font-mono text-sm font-bold">{fmtEur(item.unitCost)}</div>
                            </div>
                            <div className="bg-card rounded-lg p-3 border border-border">
                              <div className="text-[10px] text-muted-foreground uppercase">Uso Diario Est.</div>
                              <div className="font-mono text-sm font-bold">{fmt(item.dailyUsage, 1)} uds</div>
                            </div>
                            <div className="bg-card rounded-lg p-3 border border-border">
                              <div className="text-[10px] text-muted-foreground uppercase">Stock Seguridad</div>
                              <div className="font-mono text-sm font-bold">{fmt(item.safetyStock)} uds</div>
                            </div>
                            <div className="bg-card rounded-lg p-3 border border-border">
                              <div className="text-[10px] text-muted-foreground uppercase">Punto de Reorden</div>
                              <div className="font-mono text-sm font-bold">{fmt(item.reorderPoint)} uds</div>
                            </div>
                            <div className="bg-card rounded-lg p-3 border border-border">
                              <div className="text-[10px] text-muted-foreground uppercase">Cobertura</div>
                              <div className="font-mono text-sm font-bold">{item.coverageDays}d ({fmt(item.coverageDays / 7, 1)} sem.)</div>
                            </div>
                          </div>
                          <div className={`mt-3 p-3 rounded-lg text-xs ${item.reorderRecommendation.startsWith('Urgente') ? 'bg-destructive/10 border border-destructive/20 text-destructive' : item.reorderRecommendation.startsWith('Reponer') ? 'bg-warning/10 border border-warning/20 text-warning' : 'bg-muted/50 border border-border text-muted-foreground'}`}>
                            <strong>Recomendación:</strong> {item.reorderRecommendation}
                          </div>
                        </td>
                      </tr>
                    )}
